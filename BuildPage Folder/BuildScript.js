$(document).ready(function () {
    ...
    console.log(localStorage.getItem("adminItems"));
    ...
    if (adminItems) {
        ...
        for (const item of json) {
            pcParts[item.partType].push(item);
        }
    }
    ...
    $(document).on("change", ".part-type", function () {
        ...
        if (partType && pcParts[partType]) {
            ...
            pcParts[partType].forEach((item) => {
                ...
                partValueSelect.append(
                    new Option(optionText, JSON.stringify(item))
                );
            });
        } else {
    ...
    async function AIResponse(parts, budget) {
        ...
        try {
            ...
            if (
                ...
                let formattedText = spacedText.replace(
                    /\*\*(.*?)\*\*/g,
                    "<b>$1</b>"
                );
                $(".botContainer").append(
