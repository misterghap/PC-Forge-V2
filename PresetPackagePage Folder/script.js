document.getElementById("country").addEventListener("change", function () {
  // When the country select element changes, run this function
  ...
  if (selectedCountry === "canada") {
    // If the selected country is Canada set the flag image to the Canada flag
    flag.src = "../PresetPackagePage Folder/Images/canada.jpg";
  } else if (selectedCountry === "usa") {
    // If the selected country is USA set the flag image to the USA flag
    flag.src = "../PresetPackagePage Folder/Images/usa.jpg";
  } else if (selectedCountry === "uk") {
    // If the selected country is UK set the flag image to the UK flag
    flag.src = "../PresetPackagePage Folder/Images/uk.jpg";
  }
});

document
  .getElementById("computer-category")
  .addEventListener("change", function () {
    // When the computer category select element changes, run this function
    ...
    if (selectedCategory === "all") {
      // If the selected category is all, show all the products
    ...
    } else if (selectedCategory === "prebuilt") {
      // If the selected category is prebuilt, show only the prebuilt products
      prebuiltProducts.forEach(function (product) {
        product.style.display = "block";
      });
      laptopProducts.forEach(function (product) {
      ...
      // If the selected category is laptop, show only the laptop products
      prebuiltProducts.forEach(function (product) {
        product.style.display = "none";
      });
      laptopProducts.forEach(function (product) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  searchInput.addEventListener("input", function () {
    // When the search input changes, run this function
    const searchTerm = searchInput.value.toLowerCase(); // Get the search input value and convert it to lowercase
    products.forEach((product) => {
      // Loop through each product
      const productName = product
        .querySelector(".product-name")
        .textContent.toLowerCase(); // Get the product name and convert it to lowercase
      const productSpecs = product
        .querySelector(".product-specs")
        .textContent.toLowerCase(); // Get the product specs and convert it to lowercase
      if (
        productName.includes(searchTerm) ||
        productSpecs.includes(searchTerm)
      ) {
        // If the product name or specs includes the search term, show the product
...
document
  ...
  .addEventListener("submit", function (event) {
    // When the new product form is submitted, run this function
...
window.addEventListener("beforeunload", function () {
  // When the window is about to unload, run this function
...
window.addEventListener("load", function () {
  ...
  if (savedProducts) {
    // If there are saved products in local storage, run this code
    const productContainer = document.querySelector(".product-container");
    savedProducts.forEach((productData) => {
      // Loop through each saved product
      const newProduct = document.createElement("div"); // Create a new product element
      newProduct.classList.add("new-product"); // Add the new-product class to the new product element
...
document
  ...
  .addEventListener("click", function () {
    // When the delete product button is clicked, run this function
    const productId = document.getElementById("delete-product-id").value; // Get the product id value
    if (productId) {
      // If the product id value is not empty, run this code
      const productElement = document.querySelector(
        `.new-product[data-value="${productId}"]`
      ); // Get the product element with the data-value attribute equal to the product id
      if (productElement) {
        // If the product element exists, run this code
...
function updateWishlistDisplay() {
  ...
  document.getElementById("wishlist").querySelector("span").textContent =
    wishlist.length;
}
